...
def save_friendship():
    ...
    if not data or not all(k in data for k in ('user1_id', 'user2_id', 'friendship_status')):
...
def get_friendship(user1_id, user2_id):  
    account = Friendship.query.filter(Friendship.user1_id == user1_id, Friendship.user2_id == user2_id).first()
    if account is None:
