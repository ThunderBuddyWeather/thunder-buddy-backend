...
def reject_friend_request(friend_id: int) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if not friendship:
            logger.warning(f"Friendship not found between {current_user_id} and {friend_id}")
            return {"message": "Friendship not found"}, 404

        if friendship.friendship_status != 'pending':
            logger.warning(f"Cannot reject friendship that is not pending (status: {friendship.friendship_status})")
            return {"message": f"Cannot reject friendship with status: {friendship.friendship_status}"}, 409
...
def unfriend(friend_id: int) -> Tuple[Dict[str, Any], int]:
    ...
    try:
        ...
        if friendship.friendship_status != 'accepted':
            logger.warning(f"Cannot unfriend with status: {friendship.friendship_status}")
