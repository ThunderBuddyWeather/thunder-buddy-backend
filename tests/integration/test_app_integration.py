...
def app():
    ...
    app.config.update({
        'TESTING': True,
        'SQLALCHEMY_DATABASE_URI': os.getenv('DATABASE_URL', '').replace('@db:', '@localhost:'),
...
def test_friendship_flow(client, test_user, auth_headers):
    ...
    response = client.put(f'/api/friends/accept/{test_user.user_id}', headers=friend_headers)
    assert response.status_code == 200
...
def test_error_handling(client, auth_headers):
    ...
    response = client.post('/api/user/register', data='invalid json', content_type='application/json')
