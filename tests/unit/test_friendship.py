...
class TestFriendship:
    ...
    def test_send_friend_request(self):
        ...
        response = self.client.post(f'/api/friends/request/{self.user2["user_id"]}', headers=self.get_auth_headers(self.user1))
        assert response.status_code == 201

    def test_accept_friend_request(self):
        """Test accepting friend request"""
        # Send request first
        self.client.post(f'/api/friends/request/{self.user2["user_id"]}', headers=self.get_auth_headers(self.user1))
        
        # Accept request
        response = self.client.put(f'/api/friends/accept/{self.user1["user_id"]}', headers=self.get_auth_headers(self.user2))
    ...
    def test_reject_friend_request(self):
        ...
        self.client.post(f'/api/friends/request/{temp_user2["user_id"]}', 
                         headers=self.get_auth_headers(temp_user1))
    
        # Reject request - user2 rejects user1's request
        response = self.client.put(f'/api/friends/reject/{temp_user1["user_id"]}', 
    ...
    def test_reject_friend_request_wrong_status(self):
        ...
        # Send request
        self.client.post(f'/api/friends/request/{temp_user2["user_id"]}', headers=self.get_auth_headers(temp_user1))
        
        # Accept request
        self.client.put(f'/api/friends/accept/{temp_user1["user_id"]}', headers=self.get_auth_headers(temp_user2))
        
        # Try to reject an already accepted request
        response = self.client.put(f'/api/friends/reject/{temp_user1["user_id"]}', headers=self.get_auth_headers(temp_user2))
    ...
    def test_get_friends_list(self):
        ...
        self.client.post(f'/api/friends/request/{self.user2["user_id"]}', headers=self.get_auth_headers(self.user1))
        self.client.put(f'/api/friends/accept/{self.user1["user_id"]}', headers=self.get_auth_headers(self.user2))
    
        # Get friends list
        response = self.client.get('/api/friends', headers=self.get_auth_headers(self.user1))
        assert response.status_code == 200
    ...
    def test_unfriend(self):
        ...
        # Send friend request
        response = self.client.post(f'/api/friends/request/{temp_user2["user_id"]}', 
                            headers=self.get_auth_headers(temp_user1))
        assert response.status_code == 201
        
        # Accept friend request
        response = self.client.put(f'/api/friends/accept/{temp_user1["user_id"]}', 
                          headers=self.get_auth_headers(temp_user2))
        assert response.status_code == 200
        
        # Verify friendship exists
        response = self.client.get('/api/friends', headers=self.get_auth_headers(temp_user1))
        assert response.status_code == 200
        assert len(response.json['friends']) == 1
        
        # Unfriend
        response = self.client.delete(f'/api/friends/{temp_user2["user_id"]}', 
                             headers=self.get_auth_headers(temp_user1))
    ...
    def test_unfriend_not_found(self):
        ...
        response = self.client.delete(f'/api/friends/{self.user2["user_id"]}', headers=self.get_auth_headers(self.user1))
