...
def test_register_missing_fields(client):
    ...
    response = client.post("/api/user/register", json={"email": "new@example.com"})
...
def test_register_invalid_email(client):
    ...
    response = client.post(
        "/api/user/register",
        json={"email": "invalid-email", "password": "password123", "name": "New User"},
    )
    assert response.status_code == 400
...
def test_register_short_password(client):
    ...
    response = client.post(
        "/api/user/register",
        json={"email": "new@example.com", "password": "12345", "name": "New User"},
    )
    assert response.status_code == 400
...
def test_register_short_name(client):
    ...
    response = client.post(
        "/api/user/register",
        json={"email": "new@example.com", "password": "password123", "name": "A"},
    )
    assert response.status_code == 400
...
def test_login_success(client, test_user):
    ...
    response = client.post(
        "/api/user/login", json={"email": "test@example.com", "password": "password123"}
    )
...
def test_update_profile_not_found(client, auth_headers):
    ...
    headers = auth_headers(999)
    response = client.put(
        "/api/user/profile", headers=headers, json={"name": "Updated Name"}
    )
    assert response.status_code == 404
    assert "User not found" in response.json["message"]
...
def test_update_profile_invalid_fields(client, test_user, auth_headers):
    ...
    headers = auth_headers(test_user)
    response = client.put(
        "/api/user/profile", headers=headers, json={"invalid_field": "value"}
    )
    assert response.status_code == 400
...
def test_update_profile_short_name(client, test_user, auth_headers):
    ...
    headers = auth_headers(test_user)
    response = client.put("/api/user/profile", headers=headers, json={"name": "A"})
    assert response.status_code == 400
    assert "Name must be at least 2 characters long" in response.json["message"]
