...
try:
    ...
    app.config["JWT_SECRET_KEY"] = os.environ.get(
        "JWT_SECRET_KEY", "dummy-secret-for-swagger"
    )
        ...
        try:
            ...
            for item_name in dir(module):
                ...
                if isinstance(item, Blueprint):
                    ...
                    logger.info(
                        f"Registering blueprint {item_name} with prefix {url_prefix}"
                    )
...
def infer_field_type(field_name, source_code):
    ...
    if field_name.endswith("_id") or field_name == "id":
        ...
        or field_name.startswith("has_")
        or field_name in ["active", "enabled", "verified"]
    ):
        field_type = "boolean"
                ...
                if f"{var_name}[" in next_line or f"{var_name}.get(" in next_line:
                    try:
                        if f"{var_name}[" in next_line:
                            ...
                            field = (
                                next_line.split(f"{var_name}.get(")[1]
                                .split(")")[0]
                                .strip("\"'")
                            )
                            # Skip 'field' as it's a variable name in the loop
                            if (
                                ...
                                and field not in required_fields
                            ):
        ...
        for line in source.split("\n"):
            if "request.args.get" in line:
                try:
                    ...
                    is_required = default_value is None or (
                        "if not" in source and param_name in source
                    )
                    ...
                    logger.warning(
                        f"Failed to parse parameter from line: {line}. Error: {str(e)}"
                    )
        ...
        if "request.json" in source or "request.get_json()" in source:
            ...
            for line in source.split("\n"):
                ...
                if "request.get_json()" in line and "=" in line:
                    ...
                    for next_line in source.split("\n"):
                        if (
                            f"{var_name}[" in next_line
                            or f"{var_name}.get(" in next_line
                        ):
                            try:
                                if f"{var_name}[" in next_line:
                                    field = (
            ...
            if required_fields or optional_fields:
                ...
                for field in required_fields + optional_fields:
                    ...
                    clean_field = (
                        field.strip("'").strip('"').strip(",").strip(":").strip()
                    )
                    ...
                    if (
                        ...
                        match = re.search(
                            r'[\'"]?([a-zA-Z_][a-zA-Z0-9_]*)[\'"]?', clean_field
                        )
                        if match:
...
def analyze_responses(func) -> Dict[str, Any]:
    ...
    try:
                ...
                try:
                    ...
                    if "{" in line and "}" in line:
                        ...
                        if response_str.startswith("{") and ":" in response_str:
                            ...
                            fields = [
                                f.strip().strip("\"'")
                                for f in response_str.replace("{", "")
                                .replace("}", "")
                                .split(",")
                                if ":" in f
                            ]
                            field_names = [
                                f.split(":")[0].strip().strip("\"'") for f in fields
                            ]
                            ...
                            for field in field_names:
                                if field:  # Skip empty field names
                                    ...
                                    example_obj[field] = generate_example_value(
                                        field, field_type
                                    )
        ...
        if "400" not in responses and (
            'return jsonify({"error"' in source or ", 400" in source
        ):
        ...
        with app.test_request_context():
                    ...
                    for method in rule.methods - {"HEAD", "OPTIONS"}:
                        ...
                        if method_lower not in route_methods[path]:
                            try:
                                ...
                                logger.warning(
                                    f"Could not find view function for {rule.endpoint}"
                                )
            ...
            for path, methods in route_methods.items():
                ...
                for arg in [
                    ...
                    matching_rule = next(
                        (r for r in app.url_map.iter_rules() if str(r) == path), None
                    )

                    if matching_rule and arg in matching_rule._converters:
                        arg_type = matching_rule._converters[
                            arg
                        ...
                        for method_spec in path_spec.values():
                            path_param_exists = any(
                                p.get("name") == arg and p.get("in") == "path"
